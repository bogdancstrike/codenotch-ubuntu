...
Antigravity has two clients on Linux and both are recognised: the IDE and the `agy` CLI. Detection prefers a running language server on `127.0.0.1` (same-uid processes only, ports read from their own sockets), then the CLI's Google login held in the session keyring. Ordinary polling only checks that the keyring item exists; the secret is retrieved only when a request will actually be made. When Google declines to publish the allowance to a third-party client, that is surfaced as "signed in, no allowance published" rather than a missing sign-in. Upstream's transcript-count fallback is still not ported. Unsupported or absent quota is reported as unavailable, never as a percentage.
